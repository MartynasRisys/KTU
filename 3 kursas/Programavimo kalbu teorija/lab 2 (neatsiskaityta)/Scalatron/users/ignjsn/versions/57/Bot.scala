...
object ControlFunction
{
    ...
    def analyzeViewAsMaster(bot: Bot) = {
        ...
        var mapValues = ofDim[Int](31,31)
        var closedList = ofDim[Bool](31,31)
        ...
        for(i <- 0 until cellCount) {
            ...
            if(cellRelPos.isNonZero) {
                ...
                var currentX = i % 31 
                var currentY = i / 31
                val distance = src.distanceTo(currentX, currentY)
                //bot.log(currentY.toString)
                closedList(currentX)(currentY) = false
                map(currentX)(currentY) = cells(i) match {
                    ...
                    case 'B' => // good beast: valuable, but runs away
                       // bot.log(dest.distanceTo(XY(currentX, currentY)).toString)
                        if (!dest.isDefined || src.distanceTo(dest) > src.distanceTo(XY(currentX, currentY))) dest = dest.update(currentX, currentY)
                        0
