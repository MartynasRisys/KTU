...
object ControlFunction
{
    def forMaster(bot: Bot) {
        ...
        val collectorsDeployed = bot.inputAsIntOrElse("collectorsDeployed", 0)
        ...
        if(collectorsDeployed < 1 && bot.energy > 100) { // deployCollector?
            bot.spawn(XY.Down, "mood" -> "Collector")
            bot.set("collectorsDeployed" -> 1)
        }
    }
    ...
    def reactAsCollector(bot: MiniBot) {
        bot.log(bot.offsetToMaster.x.toString + " x to master")
        bot.log(bot.offsetToMaster.y.toString + " y to master")
        bot.log(bot.energy.toString + " energy")
        if (bot.energy > 1500 && bot.offsetToMaster.x < 10 && bot.offsetToMaster.x > -10 && bot.offsetToMaster.y < 10 && bot.offsetToMaster.y > -10) {
            val (directionValue, nearestEnemyMaster, nearestEnemySlave) = analyzeViewAsMaster(bot, XY(10 + bot.offsetToMaster.y, 10 + bot.offsetToMaster.x))
            bot.move(directionValue)
            bot.log("going to master")
        }
        else {
            val (directionValue, nearestEnemyMaster, nearestEnemySlave) = analyzeViewAsMaster(bot)
            bot.move(directionValue)
    ...
    /** Analyze the view, building a map of attractiveness for the 45-degree directions and
      ...
      */
    def analyzeViewAsMaster(bot: Bot, destination: XY = XY(-1, -1)) = {
        ...
        bot.log(dest.isDefined.toString + " is defined?")
        ...
        for(i <- 0 until cellCount) {
            ...
            if(cellRelPos.isNonZero) {
                ...
                if (currentCOL == center && currentROW == center) bot.log("as tevas")
                val additionalF = cells(i) match {
                    ...
                    case 'B' => // good beast: valuable, but runs away
                        if ((!dest.isDefined || src.distanceTo(dest) > src.distanceTo(XY(currentCOL, currentROW))) && !destination.isDefined)
                        {
                            dest = dest.update(currentCOL, currentROW)
                            bot.log("Updated")
                        }
                        100
