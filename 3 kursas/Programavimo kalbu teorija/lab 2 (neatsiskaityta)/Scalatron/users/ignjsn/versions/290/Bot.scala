    ...
    def analyzeViewAsMaster(bot: Bot) = {
        ...
        if (!dest.isDefined) 
        {
            ...
            while(!found) {
                ...
                if (!found)
                {
                  for (i <- yOffsetTop until n) {
                    bot.log(cellDetails(i)(xOffsetRight).toString)
                    if (cellDetails(i)(xOffsetRight).f == 100)
                    {
                       // found = true
                    }
                  }
                ...
                if (!found)
                {
                  iterator = n - xOffsetRight
                  for (i <- xOffsetRight until n) {
