...
object ControlFunction
{
    ...
    { 
        bot.log(cellDetails(15)(15).toString)
        bot.log("\nThe Path is "); 
        var row = dest.y; 
        var col = dest.x; 
        ...
        { 
            Path += XY(col, row) 
            var temp_row: Int = cellDetails(col)(row).parentROW; 
            var temp_col: Int = cellDetails(col)(row).parentCOL; 
            row = temp_row; 
            col = temp_col; 
        } 
        ...
        { 
            var p = Path.tail
            Path -= p
            bot.log("-> " + p.x.toString + " " + p.y.toString); 
        } 
    }
        ...
        while(!openList.isEmpty) {
            ...
            if (XY.isValid(currentCOL-1, currentROW) == true)
            {
                ...
                // Set the Parent of the destination cell 
                    ...
                    bot.log("Found")
                    tracePath(bot, dest, cellDetails)
                    foundDest = true
                ...
                { 
                   // bot.log("second layer start")
                    ...
                    fNew = gNew + hNew
                   // bot.log(fNew.toString + " fNew")
                   // bot.log(cellDetails(currentCOL-1)(currentROW).f.toString + " currentF")
